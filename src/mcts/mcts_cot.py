...
class ScriptArguments:
    """Arguments for fine-tuning with SFTTrainer."""
...
def get_possible_actions_llama2(model, tokenizer, env: CrosswordsEnv, state, script_args):
    ...
    generate_ids = model.generate(
        ...
        max_new_tokens=100,
        eos_token_id=13,
        num_beams=Hyperparams.num_different_action,
        num_return_sequences=Hyperparams.num_different_action,
    )
...
def get_possible_actions_gpt(env: CrosswordsEnv, state, api_endpoint, headers):
    while True:
        try:
...
def generate_full(env: CrosswordsEnv, state, api_endpoint, headers):
    while True:
        try:
            response = requests.post(
                api_endpoint,
...
def selection(Q, N, state, action_list, c=1.0):
    ...
    return action_list[chosen_action_idx]
...
def mcts_construction(model, tokenizer, env: CrosswordsEnv, initial_state: str,
                      iteration, run_name, api_endpoint, headers):
    """Build an MCTS tree rooted at initial_state and return the best action sequence."""
    Q, N, action_map = {}, {}, {}
    ...
    for rollout_idx in range(Hyperparams.rollout_num):
        ...
        # Selection: traverse existing tree nodes
        ...
        if env.answered(current_state):
            ...
            if np.random.rand() < gpt_generation_prob:
                possible_actions = get_possible_actions_gpt(env, current_state, api_endpoint, headers)
            else:
            ...
            # Simulation: roll out each action to completion and evaluate
            for action in possible_actions:
                next_state = current_state + action + '\n'
                if env.answered(next_state):
                    final_state = next_state
                else:
                    generation = generate_full(env, next_state, api_endpoint, headers)
                    final_state = next_state + generation + '\n'
                reward = env.reward(final_state)

                update_value(
                    Q, N,
                    history_state_list + [current_state],
                    history_action_list + [action],
                    reward,
                )

    # Extract greedy best-action sequence
...
def rollout_once(model, tokenizer, env: CrosswordsEnv, iteration, run_index,
                 api_endpoint, headers, run_name='train'):
    current_state = ''

    start = time.time()
    for depth in range(Hyperparams.depth_limit):
        best_action = mcts_construction(model, tokenizer, env, current_state,
                                        iteration, run_name, api_endpoint, headers)
        current_state = current_state + best_action
        if env.answered(current_state):
            break
...
def finetune_llama2(model, iteration, env_name, script_args):
    data_files = [
        f'saved_generation/{env_name}/train/iteration-{iteration}/data-{dataset_idx}.txt'
...
def evaluate_model(model, iteration, tokenizer, validation_env: CrosswordsEnv,
    ...
    for task_id in range(validation_length):
        ...
        generation = rollout_once(model, tokenizer, validation_env, iteration, task_id,
                                  api_endpoint, headers, run_name='test')
...
def cot_mcts(model, tokenizer, env: CrosswordsEnv, validation_env: CrosswordsEnv,
             script_args, api_endpoint, headers):
    """Main MCTS + fine-tuning loop."""
    performance_list = []

    for iteration in range(Hyperparams.train_iterations):
        for dataset_idx in range(Hyperparams.data_num_per_training):
            env.reset_random()
            rollout_once(model, tokenizer, env, iteration, dataset_idx,
                         api_endpoint, headers, run_name='train')

        finetune_llama2(model, iteration, env.env_name, script_args)
...
def build_llama_model(script_args):
    ...
    print('Model loaded:', script_args.model_name)
...
if __name__ == '__main__':
    ...
    model, tokenizer = build_llama_model(script_args)

    env = CrosswordsEnv()
    validation_env = CrosswordsEnv()

    cot_mcts(model, tokenizer, env, validation_env, script_args, API_ENDPOINT, headers)
